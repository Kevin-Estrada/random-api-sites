...
async function getAllDrinks() {
  let tequilaData = await fetch(
    `https://www.thecocktaildb.com/api/json/v1/1/filter.php?i=tequila`
  );
  let tequilaJsonData = await tequilaData.json();

  let liquerDataArrData = await getAllDrinkCategories('Liqueur');
  let liquerArrJsonData = await liquerDataArrData.json();

  let liquerDataArrData = await getAllDrinkCategories('Liqueur');
  let liquerArrJsonData = await liquerDataArrData.json();
}

async function getTequillaDrinks() {
...
async function tequilaHelperFunc(drinkChoice) {
  let data = getTequillaDrinks(drinkChoice);
